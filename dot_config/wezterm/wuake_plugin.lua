...
local data_dir = (os.getenv("XDG_DATA_HOME") or wezterm.home_dir ..
                     "/.local/share") .. "/wezterm/"
...
local function read_pidfile() return read_file(pidfile_path) end

local function log_pidfile()
    wezterm.log_info("pid: " .. (read_pidfile() or "nil"))
end

local function write_pidfile() return write_file(pidfile_path, pid) end
...
function module.setup(params)
    ...
    wezterm.on("update-status", function(window)
        ...
        if this_proc_info.argv[2] ~= "connect" or this_proc_info.argv[3] ~=
            params.domain then
            wezterm.log_info("not a wuake window")
        else
            local now = tonumber(wezterm.time.now():format("%s%.f"))
            local pid_info = wezterm.procinfo.get_info_for_pid(read_pidfile() or
                                                                   0)
            ...
            if (not (pid_info and pid_info.name == "wezterm-gui") and
                (last_exit and now - 0.5 > tonumber(last_exit))) then
                wezterm.log_info("starting wuake...")
                write_pidfile()

                local screens = wezterm.gui.screens()
                window:set_position(screens.active.x + params.margin_right,
                                    screens.active.y + params.margin_top)
                window:set_inner_size(
                    screens.active.width * params.width_factor -
                        params.margin_right - params.margin_left,
                    screens.active.height * params.height_factor -
                        params.margin_bottom - params.margin_top)
